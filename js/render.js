/* ============================================================
   МОДУЛЬ: render.js
   ОТВЕТСТВЕННОСТЬ: Рендеринг UI — дашборд, склад, движения,
   SQL-консоль
   ФИЗИКА ПРОЦЕССА: получает данные из data-layer.js и
   analytics.js, формирует HTML для отображения
   ============================================================ */
...
/*
 * Рендер дашборда: KPI-карточки + критичные позиции + последние движения
 */
export function renderDashboard() {
  ...
  document.getElementById('kpiGrid').innerHTML = `
    <div class="kpi">
      <div class="kpi-label">Позиций в норме</div>
      <div class="kpi-value" style="color:var(--norm);">${counts.norm}</div>
      <div class="kpi-delta up">из ${counts.total} номенклатур</div>
    </div>
    <div class="kpi warn">
      <div class="kpi-label">Требуют внимания</div>
      <div class="kpi-value" style="color:var(--warn);">${counts.warn}</div>
      <div class="kpi-delta">ниже точки заказа</div>
    </div>
    <div class="kpi crit">
      <div class="kpi-label">Критический дефицит</div>
      <div class="kpi-value" style="color:var(--crit);">${counts.crit}</div>
      <div class="kpi-delta down">риск остановки производства</div>
    </div>
    <div class="kpi">
      <div class="kpi-label">Движений за 7 дней</div>
      <div class="kpi-value">${recent.length}</div>
      <div class="kpi-delta">операций проведено</div>
    </div>
  `;
  ...
  document.getElementById('criticalList').innerHTML = criticalItems.length === 0
    ...
    : criticalItems.map(x => `
      <div style="padding:12px;border-left:4px solid var(--${x.status});background:var(--panel-2);margin-bottom:8px;border-radius:4px;">
        <div style="font-weight:600;font-size:13px;">${x.name}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:4px;">
          Остаток: <b>${x.stock}</b> · ROP: ${x.rop} · Дней до истощения: <b>${daysUntilStockout(x.art)}</b>
        </div>
      </div>
    `).join('');
  ...
  document.getElementById('recentMovements').innerHTML = recent.map(row => {
    ...
    return `
      <div style="padding:8px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;font-size:13px;">
        <div>
          <div>${typeLabel} · <b>${itemName}</b></div>
          <div style="font-size:11px;color:var(--muted);">${date}</div>
        </div>
        <div style="font-weight:600;color:${color};">${delta > 0 ? '+' : ''}${delta}</div>
      </div>
    `;
  }).join('');
}
...
/*
 * Рендер таблицы склада с фильтрами
 */
export function renderStock() {
  ...
  const filtered = items
    .map(row => {
      ...
      return {
        art, name, cat, unit, avg,
        stock: getCurrentStock(art),
        rop: calcROP(art),
        status: getStatus(art),
        days: daysUntilStockout(art)
      };
    })
    .filter(x => {
  ...
  document.getElementById('stockTable').innerHTML = filtered.map(x => `
    <tr>
      <td><code style="background:#f1f5f9;padding:2px 6px;border-radius:3px;font-size:12px;">${x.art}</code></td>
      <td><b>${x.name}</b></td>
      <td>${categoryLabel[x.cat]}</td>
      <td class="text-right"><b>${x.stock}</b> ${x.unit}</td>
      <td class="text-right">${x.rop}</td>
      <td class="text-right">${x.avg}</td>
      <td><span class="status ${x.status}"><span class="status-dot"></span>${x.status === 'norm' ? 'Норма' : x.status === 'warn' ? 'Внимание' : 'Критично'}</span></td>
      <td class="text-right" style="font-weight:600;color:${x.days < 7 ? 'var(--crit)' : x.days < 14 ? 'var(--warn)' : 'var(--text)'};">${x.days}</td>
    </tr>
  `).join('');
}

/*
 * Рендер вкладки "Движения ТМЦ"
 */
export function renderMovements() {
  const items = getItems();
  
  // Заполняем селект номенклатуры
  ...
  document.getElementById('movementsTable').innerHTML = movements.map(row => {
    ...
    return `
      <tr>
        ...
        <td><b>${itemName}</b></td>
        <td class="text-right" style="font-weight:600;color:${delta > 0 ? 'var(--norm)' : 'var(--crit)'};">${delta > 0 ? '+' : ''}${delta}</td>
