// lib/store.js — v12: универсальное извлечение тела из get() (новый SDK)
...
async function blobToText(blob) {
  ...
  if (!text && blob.url) {
    text = await tryText(async () => {
      const r = await fetch(blob.url, {
        headers: { authorization: `Bearer ${process.env.PRIV_READ_WRITE_TOKEN}` },
      });
      return r.ok ? await r.text() : '';
    });
  }
