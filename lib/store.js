// lib/store.js — финальная версия (storeId + PRIV-токен + access private)
...
const blobOpts = () => {
  ...
  if (!token || !storeId) throw new Error('Не заданы PRIV_READ_WRITE_TOKEN / PRIV_STORE_ID в Env');
...
export async function saveState(state) {
  ...
  await put(STATE_KEY, JSON.stringify(state), {
    ...blobOpts(), addRandomSuffix: false, contentType: 'application/json',
  });
}
