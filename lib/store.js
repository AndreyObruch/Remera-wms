// lib/store.js — v10: усиленный loadState (пустой/битый JSON = чистый старт)
...
export async function loadState() {
  ...
  if (!text || !text.trim()) return defaultState(); // пустой файл = чистый старт

  try {
    return { ...defaultState(), ...JSON.parse(text) };
  } catch (e) {
    return defaultState(); // битый JSON = чистый старт
  }
}
