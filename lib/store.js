...
// Опция access убрана — тип store определяется токеном автоматически.
...
export async function storageSelfTest() {
  await put('wms2/debug.txt', 'ok', { addRandomSuffix: false, token: token() });
  const b = await get('wms2/debug.txt', { token: token() });
